...
static void
glamor_intersect_box(BoxPtr box1,
                     BoxPtr box2)
{
...
/**
 * Copies overlapping areas of a pixmap by using glBlitFramebuffer, if
 * GL_MESA_overlapping_blit is supported.
 */
static Bool
                         ...
                         void *closure)
{
    ...
    /* This path is only for overlapping (src == dst) copies, which
     * simplifies our loops a bit.
     */
    ...
    /* Walk over the dst pixmaps in the order we should be doing our
     * drawing so that all reads of a pixel happen before it's
     * written.
     */
        ...
        for (int dst_box_xi = 0; dst_box_xi < wcnt; dst_box_xi++) {
            ...
            for (int src_box_yi = 0; src_box_yi < hcnt; src_box_yi++) {
                ...
                for (int src_box_xi = 0; src_box_xi < wcnt; src_box_xi++) {
                    ...
                    /* The src FBO's box, shifted down by the delta
                     * that we're copying across.
                     */
                    ...
                    /* The incoming boxes should already be sorted in
                     * the order that they need to be copied.
                     */
                    for (int draw_box_i = 0; draw_box_i < nbox; draw_box_i++) {
                        ...
                        /* Intersect this box to be drawn with the
                         * boxes for the FBOs that we're binding.
                         */
                        ...
                        glBindFramebuffer(GL_READ_FRAMEBUFFER, src_fbo->fb);
                        glBindFramebuffer(GL_DRAW_FRAMEBUFFER, dst_fbo->fb);
                        glBlitFramebuffer(draw_box.x1 + dx,
                                          draw_box.y1 + dy,
                                          draw_box.x2 + dx,
                                          draw_box.y2 + dy,
                                          draw_box.x1,
